...
class ChatServer implements MessageComponentInterface
{
    ...
    private function handleJoinVoice(ConnectionInterface $from,array $data,array &$meta):void
    {
        ...
        // Leave any previous voice room first and notify only that room.
        $previousVoice=(int)($meta['voice_channel_id']??0);
        if($previousVoice&&$previousVoice!==$channelId){
            $oldRemaining=$this->voiceRooms[$previousVoice]??[];
            $this->voiceRooms[$previousVoice]=array_values(array_filter(
                $oldRemaining,
                fn($p)=>(int)$p['user_id']!==$uid
            ));
            $leavePayload=json_encode([
                'type'=>'voice_leave',
                'user_id'=>$uid,
                'username'=>$meta['username'],
                'channel_id'=>$previousVoice
            ]);
            foreach($this->voiceRooms[$previousVoice]??[] as $p){
                foreach($this->userConns[(int)$p['user_id']]??[] as $peerConn){
                    try{$peerConn->send($leavePayload);}catch(\Exception){}
                }
            }
            if(empty($this->voiceRooms[$previousVoice]))unset($this->voiceRooms[$previousVoice]);
        }
        ...
        $this->voiceRooms[$channelId]=array_values(array_filter(
            $this->voiceRooms[$channelId],
            fn($p)=>(int)$p['user_id']!==$uid
        ));
        $this->voiceRooms[$channelId][]=[
        ...
        // Voice events are room-scoped. Never broadcast join/leave events to
        // unrelated voice channels or servers.
