...
try {
    ...
    $access = $db->prepare(
        'SELECT 1
         ...
         WHERE p.id = :pid AND m.is_deleted = 0
         LIMIT 1'
    );
    $access->execute([':uid' => $user['id'], ':pid' => $pollId]);
    if (!$access->fetchColumn()) {
        http_response_code(403);
        echo json_encode(['error' => 'You do not have access to this poll']);
        exit;
    }
    ...
    echo json_encode([
        ...
        'options' => array_map(fn($o) => ['id' => (int)$o['id'], 'text' => $o['option_text'], 'vote_count' => (int)$o['vote_count']], $options),
    ]);
} catch (Throwable $e) {
