...
function extra_flashcards(PDO $db, int $uid, string $uname, int $cid, string $action, array $body): never
{
    match ($action) {
        ...
        'get_deck' => (function () use ($db, $uid, $cid, $body) {
            ...
            $deck = $d->fetch();
            if (!$deck) json_fail('Deck not found', 404);
        ...
        'create_deck' => (function () use ($db, $uid, $uname, $cid, $body) {
            ...
            if (!empty($body['cards']) && is_array($body['cards'])) {
                ...
                foreach (array_slice($body['cards'], 0, 200) as $i => $card) {
                    $cs->execute([
                        ':d' => $did,
                        ':f' => mb_substr($card['front'] ?? '', 0, 1000),
                        ':b' => mb_substr($card['back'] ?? '', 0, 1000),
                        ':h' => mb_substr($card['hint'] ?? '', 0, 500),
                        ':p' => $i,
                        ':u' => $uid
                    ]);
                }
        ...
        'add_card' => (function () use ($db, $uid, $uname, $cid, $body) {
            ...
            $deck = $db->prepare("SELECT id FROM collab_decks WHERE id=:d AND channel_id=:c LIMIT 1");
            $deck->execute([':d' => $did, ':c' => $cid]);
            if (!$deck->fetch()) json_fail('Deck not found', 404);
        ...
        'rate_card' => (function () use ($db, $uid, $cid, $body) {
            ...
            $card = $db->prepare("SELECT fc.id FROM collab_flashcards fc JOIN collab_decks d ON d.id=fc.deck_id WHERE fc.id=:card AND d.channel_id=:channel LIMIT 1");
            $card->execute([':card' => $cid2, ':channel' => $cid]);
            if (!$card->fetch()) json_fail('Card not found', 404);
    ...
    throw new LogicException('Flashcards dispatch unexpectedly returned');
}
...
function extra_mindmap(PDO $db, int $uid, string $uname, int $cid, string $action, array $body): never
{
    ...
    match ($action) {
        'get' => (function () use ($ensureMap) {
            json_ok(['map' => $ensureMap()]);
        })(),
    ...
    throw new LogicException('Mindmap dispatch unexpectedly returned');
}
...
function extra_review(PDO $db, int $uid, string $uname, int $cid, string $action, array $body): never
{
    match ($action) {
        ...
        'get' => (function () use ($db, $cid, $body) {
            $rid = (int)($body['request_id'] ?? $_GET['request_id'] ?? 0);
            if (!$rid) json_fail('request_id required');
        ...
        'add_feedback' => (function () use ($db, $uid, $uname, $cid, $body) {
            ...
            if (!$rid) json_fail('request_id required');
            $comment = mb_substr(trim($body['comment'] ?? ''), 0, 3000);
            if (!$comment) json_fail('comment required');
    ...
    throw new LogicException('Review dispatch unexpectedly returned');
}
...
function extra_summary(PDO $db, int $uid, string $uname, int $cid, string $action, array $body): never
{
    ...
    throw new LogicException('Summary dispatch unexpectedly returned');
}
...
function extra_goals(PDO $db, int $uid, string $uname, int $cid, string $action, array $body): never
{
    match ($action) {
        ...
        'create' => (function () use ($db, $uid, $uname, $cid, $body) {
            ...
            $db->prepare("INSERT INTO collab_goals (channel_id,user_id,title,description,scope,target_date) VALUES(:c,:u,:t,:d,:s,:td)")
                ->execute([
                    ...
                    ':u' => $uid,
                    ':t' => $title,
                    ':d' => mb_substr($body['description'] ?? '', 0, 1000),
                    ':s' => $scope,
                    ':td' => $body['target_date'] ?? null
                ]);
        ...
        'update_progress' => (function () use ($db, $uid, $uname, $cid, $body) {
            $gid = (int)($body['goal_id'] ?? 0);
            if (!$gid) json_fail('goal_id required');
        ...
        'toggle_milestone' => (function () use ($db, $uid, $cid, $body) {
            $mid = (int)($body['milestone_id'] ?? 0);
            if (!$mid) json_fail('milestone_id required');
        ...
        'react' => (function () use ($db, $uid, $cid, $body) {
            ...
            $goal = $db->prepare("SELECT id FROM collab_goals WHERE id=:g AND channel_id=:c LIMIT 1");
            $goal->execute([':g' => $gid, ':c' => $cid]);
            if (!$goal->fetch()) json_fail('Goal not found', 404);
    ...
    throw new LogicException('Goals dispatch unexpectedly returned');
}
...
function extra_resources(PDO $db, int $uid, string $uname, int $cid, string $action, array $body): never
{
    ...
    match ($action) {
        ...
        'vote' => (function () use ($db, $uid, $cid, $body) {
            ...
            if (!$rid) json_fail('resource_id required');
            $resource = $db->prepare("SELECT id FROM collab_resources WHERE id=:r AND channel_id=:c LIMIT 1");
            $resource->execute([':r' => $rid, ':c' => $cid]);
            if (!$resource->fetch()) json_fail('Resource not found', 404);
            // Toggle vote
        ...
        'comment' => (function () use ($db, $uid, $uname, $cid, $body) {
            $rid     = (int)($body['resource_id'] ?? 0);
            if (!$rid) json_fail('resource_id required');
            $comment = mb_substr(trim($body['comment'] ?? ''), 0, 1000);
            if (!$comment) json_fail('comment required');
            $resource = $db->prepare("SELECT id FROM collab_resources WHERE id=:r AND channel_id=:c LIMIT 1");
            $resource->execute([':r' => $rid, ':c' => $cid]);
            if (!$resource->fetch()) json_fail('Resource not found', 404);
        ...
        'get_comments' => (function () use ($db, $cid, $body) {
            $rid = (int)($body['resource_id'] ?? $_GET['resource_id'] ?? 0);
            if (!$rid) json_fail('resource_id required');
            $s = $db->prepare("SELECT c.*,u.username FROM collab_resource_comments c JOIN users u ON u.id=c.user_id JOIN collab_resources r ON r.id=c.resource_id WHERE c.resource_id=:r AND r.channel_id=:c ORDER BY c.created_at");
    ...
    throw new LogicException('Resources dispatch unexpectedly returned');
}
