...
try {
    ...
    $stmt = $db->prepare('SELECT id, channel_id, document_key, storage_path, file_type FROM collab_documents WHERE id = :id LIMIT 1');
    $stmt->execute([':id' => $id]);
    ...
    // 2 = ready for saving; 6 = force-save result.
    if (in_array($status, [2, 6], true)) {
        ...
        try {
            ...
            if (!rename($tmp, $target)) {
                if (!copy($tmp, $target)) throw new RuntimeException('Could not replace document.');
                @unlink($tmp);
            }
