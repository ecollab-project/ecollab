...
async function initFacilitatorServerPages(){try{const s=await facOwnedServers();facServerPicker('page-resources',s,loadFacResources);facServerPicker('page-files',s,loadFacWorkspace);facServerPicker('page-useractivity',s,loadFacActivity);facServerPicker('page-reports',s,()=>toast('Reports are scoped to the selected server','info','🚩'));facServerPicker('page-banned',s,()=>toast('Banned users are scoped to the selected server','info','🚫'));facServerPicker('page-polls',s,()=>toast('Polls and quizzes will use channels from the selected server','info','📊'));}catch(e){console.error('Facilitator server pages:',e);}}
