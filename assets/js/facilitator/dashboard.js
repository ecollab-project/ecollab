...
function facResolveMenu(sid,reportId,uid){
  const choice=prompt('Resolve report:\n1 = Suspend for 24 hours (view-only server access)\n2 = Mute for 24 hours (chat channels only)\n3 = Ban from server\n\nEnter 1, 2, or 3:');
  if(choice==='1')facModerate(sid,uid,'suspend',reportId,1440);
  else if(choice==='2')facModerate(sid,uid,'mute',reportId,1440);
  else if(choice==='3')facModerate(sid,uid,'ban',reportId,null);
  else if(choice!==null)toast('Choose 1, 2, or 3','error','❌');
}
