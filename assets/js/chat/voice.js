...
const _screenShareTrackIds = {}; // remoteUserId => screen track id (for disambiguation)
const _pendingScreenTrack = {}; // remoteUserId => true when next video track arriving is a screen share
...
let iceCandidateQueues = {};    // user_id => RTCIceCandidate[] (queued before remoteDescription is set)
...
function renderVcUser() {
  ...
  const roleLabel = rawRole
    ...
    : '';
  const roleDisplay = roleLabel ? `${roleLabel} · Joined now` : 'Joined now';
  ...
  speakingGrid.innerHTML = `
    <div class="vc-speaker-card speaking" data-user-id="${user.userId || 0}">
      ...
      <div class="sc-role">${roleDisplay}</div>
...
function addVcParticipant(user, isSpeaking = false) {
  ...
  const roleLabel = (user.role && user.role !== 'null')
  ...
  if (isSpeaking) {
    ...
    card.innerHTML = `
      ...
      <div class="sc-role">${roleLabel ? roleLabel + ' · ' : ''}Joined now</div>
      <div class="sc-mic-btn ${isMuted ? 'muted-state' : ''}" title="${isMuted ? 'Muted' : 'Speaking'}">
        ${isMuted
        ? `<svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
              <line x1="1" y1="1" x2="23" y2="23" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
              <path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/>
              <path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>`
        : `<svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/>
              <path d="M19 10v2a7 7 0 0 1-14 0v-2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
    ...
    card.innerHTML = `
      ...
      <div class="lc-sub">${roleLabel ? roleLabel + ' · ' : ''}Muted</div>
...
function _moveUserCardOnMute(isMuted) {
  ...
  if (isMuted) {
    ...
    card.innerHTML = `
      ...
      <div class="sc-role">${(() => { const r = window.ECOLLAB?.role || ''; return r ? (r.charAt(0).toUpperCase() + r.slice(1).toLowerCase() + ' · ') : ''; })()}Unmuted</div>
      <div class="sc-mic-btn" onclick="toggleVcMic()">
        <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/>
          <path d="M19 10v2a7 7 0 0 1-14 0v-2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
...
async function startStopScreenShare() {
  if (!vcScreenOn) {
    try {
      ...
      // ── Notify all peers that screen share is starting (BEFORE the renegotiation offer)
      console.log('[SCREEN DEBUG] Sending screen_share_notify active:true');
      if (window.chatSocket && window.chatSocket.readyState === WebSocket.OPEN) {
        window.chatSocket.send(JSON.stringify({ type: 'screen_share_notify', active: true }));
      } else {
        console.warn('[SCREEN DEBUG] chatSocket not open! readyState:', window.chatSocket?.readyState);
      }

      // ── Send screen track to all connected peers ──────────────────────
      const screenVideoTrack = _screenStream.getVideoTracks()[0];
      screenVideoTrack.contentHint = 'detail';
      // Use a video-only stream for the screen track — no audio track in it.
      // On the receiver side we detect: video track whose stream has no audio = screen share.
      const screenOnlyStream = new MediaStream([screenVideoTrack]);
      ...
      // Show screen share in its own section above Speaking
...
function stopScreenShare() {
  ...
  if (_screenStream) {
    const stoppedTrack = _screenStream.getVideoTracks()[0];
    // Notify peers screen share is stopping
    if (window.chatSocket && window.chatSocket.readyState === WebSocket.OPEN) {
      window.chatSocket.send(JSON.stringify({ type: 'screen_share_notify', active: false }));
    }
  ...
  console.log('[SCREEN DEBUG] stopScreenShare hiding section, _screenStream was:', !!_screenStream);
  const screenSection = document.getElementById('vcScreenSection');
...
function _createPeerConnection(remoteUserId, remoteUsername) {
  ...
  iceCandidateQueues[remoteUserId] = [];  // reset queue for this peer
  ...
  // On receiving remote tracks — differentiate audio, camera, and screen share
  pc.ontrack = ({ track, streams }) => {
    const stream = streams[0];
    if (track.kind === 'audio') {
      // Build an audio-only stream from this track and attach it
      const audioStream = new MediaStream([track]);
      remoteStreams[remoteUserId] = audioStream;
      _attachRemoteAudio(remoteUserId, remoteUsername, audioStream);
      _startRemoteVAD(remoteUserId, audioStream);
    } else if (track.kind === 'video') {
      // Most reliable detection: the screen track is sent in a video-only stream (no audio tracks).
      // Camera video is sent in localStream which always contains an audio track.
      // This requires zero flags, zero timing, works cross-browser.
      const streamHasAudio = streams[0] && streams[0].getAudioTracks().length > 0;
      const byStream = !streamHasAudio;
      const byHint = track.contentHint === 'detail';
      const byFlag = !!_pendingScreenTrack[remoteUserId];
      const byLabel = track.label.toLowerCase().includes('screen') ||
        track.label.toLowerCase().includes('window') ||
        track.label.toLowerCase().includes('tab');
      const isScreen = byStream || byHint || byFlag || byLabel;

      console.log('[SCREEN DEBUG] ontrack video fired:', {
        remoteUserId, label: track.label, contentHint: track.contentHint,
        streamHasAudio, byStream, byHint, byFlag, byLabel, isScreen,
        streamId: streams[0]?.id,
        pendingMap: JSON.stringify(_pendingScreenTrack)
      });

      if (byFlag) delete _pendingScreenTrack[remoteUserId];

      if (isScreen) {
        const screenOnlyStream = new MediaStream([track]);
        _showRemoteScreenShareSection(remoteUserId, remoteUsername, screenOnlyStream);
      } else {
        const camOnlyStream = new MediaStream([track]);
...
function _removeRemoteScreenShare(userId) {
  const card = document.querySelector(`.vc-speaker-card[data-user-id="${userId}"]`);
  if (!card) return;
  const vid = card.querySelector('.vc-screen-preview');
  if (vid) { vid.srcObject = null; vid.remove(); }
  card.classList.remove('has-screen');
  if (!card.querySelector('.vc-cam-preview')) card.classList.remove('has-camera');
}
...
function _hideRemoteScreenShareSection(userId) {
  ...
  if (screenGrid) {
    ...
    const cnt = document.getElementById('vcScreenSectionCount');
    if (cnt) cnt.textContent = String(screenGrid.querySelectorAll('.vc-screen-card').length);
    if (!screenGrid.querySelector('.vc-screen-card')) {
      const screenSection = document.getElementById('vcScreenSection');
      if (screenSection) screenSection.style.display = 'none';
    }
  }
...
async function _handleWebRtcOffer(fromUserId, fromUsername, sdp, isScreenOffer) {
  // Reuse existing peer connection for renegotiation (screen share added/removed).
  // Only create a new one if none exists yet.
  let pc = peerConnections[fromUserId];
  const isRenegotiation = !!pc;
  if (!pc) {
    pc = _createPeerConnection(fromUserId, fromUsername);
  }
  // Tag this peer so ontrack knows the next video track is a screen share
  if (isScreenOffer) {
    _pendingScreenTrack[fromUserId] = true;
  }
  try {
    await pc.setRemoteDescription(new RTCSessionDescription(sdp));
    await _drainIceCandidateQueue(fromUserId);
    ...
    console.log(`[WebRTC] Answer sent to ${fromUsername} (renegotiation: ${isRenegotiation}, screenOffer: ${isScreenOffer})`);
  } catch (err) {
...
async function _drainIceCandidateQueue(userId) {
  const pc = peerConnections[userId];
  const queue = iceCandidateQueues[userId];
  if (!pc || !queue || !queue.length) return;
  console.log(`[WebRTC] Draining ${queue.length} queued ICE candidate(s) for user ${userId}`);
  const candidates = queue.splice(0);
  for (const c of candidates) {
    try {
      await pc.addIceCandidate(c);
    } catch (err) {
      console.warn('[WebRTC] Queued ICE candidate failed:', err.message);
    }
  }
}
...
async function _handleWebRtcCandidate(fromUserId, candidate) {
  ...
  // Queue the candidate if remoteDescription isn't set yet — drain later
  if (!pc.remoteDescription) {
    if (!iceCandidateQueues[fromUserId]) iceCandidateQueues[fromUserId] = [];
    iceCandidateQueues[fromUserId].push(iceCandidate);
    console.log(`[WebRTC] Queued ICE candidate for user ${fromUserId} (remoteDescription not set yet)`);
    return;
  }
...
function handleVoicePeers(data) {
  ...
  peers.forEach(peer => {
    ...
    // Render the participant card if not already present
    const existing = document.querySelector(
      `.vc-speaker-card[data-user-id="${peer.user_id}"], .vc-listener-card[data-user-id="${peer.user_id}"]`
    );
    if (!existing) {
      addVcParticipant({
        id: peer.user_id,
...
window._hideRemoteScreenShareSection = _hideRemoteScreenShareSection;
// Expose _pendingScreenTrack so socket.js can set it on screen_share_notify
Object.defineProperty(window, '_pendingScreenTrack', { get: () => _pendingScreenTrack });
window._handleWebRtcCandidate = _handleWebRtcCandidate;
window._initiateWebRtcOffer = _initiateWebRtcOffer;

// Expose state so other modules can read it
Object.defineProperty(window, 'vcChannelId', { get: () => vcChannelId });
Object.defineProperty(window, 'peerConnections', { get: () => peerConnections });
