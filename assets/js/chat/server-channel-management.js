...
(function () {
  ...
  function serverId() {
    ...
    return Number(e.currentServerId || document.querySelector('.workspace-icon.active[data-server-id]')?.dataset.serverId || document.querySelector('.workspace-icon[data-server-id]')?.dataset.serverId || 0);
  }
  ...
  function channelName(id = channelId()) {
    const el = id ? document.querySelector(`.channel-item[data-channel-id="${id}"]`) : null;
    return String(el?.dataset.channelName || window._currentChannelMeta?.name || document.getElementById('channelTitle')?.textContent || 'channel');
  }

  function syncChannelContext(id = channelId()) {
    id = Number(id || 0);
    if (!id) return 0;
    ...
    window._currentChannelMeta = Object.assign({}, window._currentChannelMeta || {}, {id, name:channelName(id)});
    return id;
  }
  ...
  function trackChannelClicks() {
    document.addEventListener('click', e => {
      const item = e.target.closest?.('[data-channel-id]');
      if (item) syncChannelContext(Number(item.dataset.channelId || 0));
    }, true);
    syncChannelContext();
  }
  ...
  function styles() {
    ...
    s.textContent = `
      ...
      .scm-help{padding:10px 12px;margin-bottom:12px;border:1px solid rgba(168,85,247,.16);background:rgba(168,85,247,.06);border-radius:9px;color:#cbd5e1;font-size:11px;line-height:1.45}
  ...
  function close() { document.getElementById('scmModal')?.style && (document.getElementById('scmModal').style.display='none'); }
  ...
  async function openServerManager(tab='Invite') {
    const id = serverId();
    if (!id) return toast('Select a server first','error');
    context='server';
    open('Server Management',['Invite','Add Members','Members'],tab);
  ...
  async function openChannelManager(tab='Invite') {
    hideLegacyManager();
    const id=syncChannelContext();
    if(!id) return toast('Select a channel first','error');
    context='channel'; open('Channel Management',['Invite','Members'],tab);
    try { const d=await request('/API/chat/channel-members.php','list',{channel_id:id}); currentChannel=d.channel||{id,name:channelName(id)}; syncChannelContext(currentChannel.id); scmTab(tab,d); }
    catch(e){ document.getElementById('scmBody').innerHTML=`<div class="scm-empty">${esc(e.message)}</div>`; }
  }
  ...
  async function scmTab(tab,prefetched=null) {
    ...
    if(context==='server') {
      ...
      if(tab==='Add Members') return renderServerAddMembers(body);
  ...
  async function loadServerInvites(){
    ...
    try{const d=await request('/API/server/invite.php','list',{server_id:currentServer?.id||serverId()});el.innerHTML=d.invites.length?d.invites.map(i=>`<div class="scm-row"><div class="scm-info"><div class="scm-name">Invite #${i.id}</div><div class="scm-meta">${i.use_count}${i.max_uses?' / '+i.max_uses:''} uses · ${i.revoked_at?'Revoked':(i.expires_at?'Expires '+esc(i.expires_at):'Never expires')}</div></div>${!i.revoked_at?`<button class="scm-btn red" onclick="window.scmRevokeServerInvite(${i.id})">Revoke</button>`:''}</div>`).join(''):'<div class="scm-empty">No invites created yet.</div>';window.scmRevokeServerInvite=async id=>{try{await request('/API/server/invite.php','revoke',{},'POST',{invite_id:id});await loadServerInvites();}catch(e){toast(e.message,'error');}};}catch(e){el.innerHTML=`<div class="scm-empty">${esc(e.message)}</div>`;}
  }
  ...
  function renderCreatedInvite(body,url){
    const wrap=document.createElement('div');wrap.className='scm-invite';wrap.style.margin='12px 0';wrap.innerHTML=`<input class="scm-input" value="${esc(url)}" readonly><button class="scm-btn green">Copy</button>`;wrap.querySelector('button').onclick=async()=>{try{await navigator.clipboard.writeText(url);toast('Invite link copied','success');}catch(e){toast('Copy failed','error');}};body.insertBefore(wrap,body.children[2]||null);
  }
  ...
  async function loadChannelInvites(){
    ...
    try{const d=await request('/API/chat/channel-invite.php','list',{channel_id:currentChannel?.id||syncChannelContext()});el.innerHTML=d.invites.length?d.invites.map(i=>`<div class="scm-row"><div class="scm-info"><div class="scm-name">Invite #${i.id}</div><div class="scm-meta">${i.use_count}${i.max_uses?' / '+i.max_uses:''} uses · ${i.revoked_at?'Revoked':(i.expires_at?'Expires '+esc(i.expires_at):'Never expires')}</div></div>${!i.revoked_at?`<button class="scm-btn red" onclick="window.scmRevokeChannelInvite(${i.id})">Revoke</button>`:''}</div>`).join(''):'<div class="scm-empty">No channel invites yet.</div>';window.scmRevokeChannelInvite=async id=>{try{await request('/API/chat/channel-invite.php','revoke',{},'POST',{invite_id:id});await loadChannelInvites();}catch(e){toast(e.message,'error');}};}catch(e){el.innerHTML=`<div class="scm-empty">${esc(e.message)}</div>`;}
  }

  async function renderChannelMembers(body,prefetched){
    const id=currentChannel?.id||syncChannelContext();
    if(!id)return body.innerHTML='<div class="scm-empty">No channel selected.</div>';
    try{
      const data=prefetched||await request('/API/chat/channel-members.php','list',{channel_id:id});
      currentChannel=data.channel||{id,name:channelName(id)};syncChannelContext(currentChannel.id);
  ...
  function boot(){
    styles();hideLegacyManager();trackChannelClicks();headerActions();handleInviteQuery();
    const observer=new MutationObserver(()=>{hideLegacyManager();syncChannelContext();headerActions();});
