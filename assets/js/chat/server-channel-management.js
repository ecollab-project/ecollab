 ...
 * Loaded by chat.php after the existing chat/peer-matching scripts.
 */
(function () {
  'use strict';
  ...
  function activeServerId() {
    const el = document.querySelector('.workspace-icon.active[data-server-id]') || document.querySelector('.workspace-icon[data-server-id]');
    return Number(el?.dataset.serverId || 0);
  }
  function activeChannelId() {
    const el = document.querySelector('.channel-item.active[data-channel-id], .voice-channel.active[data-channel-id]');
    return Number(el?.dataset.channelId || 0);
  }
  ...
  async function openServerManager(tab='Invite') {
    const sid = activeServerId(); if (!sid) return toast('Select a server first','info');
    currentContext='server'; currentTab=tab;
    open('Server Management', ['Invite','Members'], tab);
    const info = await request('/API/server/members.php','info',{server_id:sid});
  ...
  async function openChannelManager(tab='Invite') {
    const cid=activeChannelId(); if(!cid)return toast('Select a channel first','info');
    currentContext='channel'; currentTab=tab;
    open('Channel Management', ['Invite','Members'], tab);
    const data=await request('/API/chat/channel-members.php','list',{channel_id:cid});
    currentChannel=data.channel;
    scmTab(tab,data);
  }
  window.openChannelManager = openChannelManager;
  window.openPrivateChannelManager = openChannelManager;
  ...
  async function scmTab(tab, prefetched=null) {
    ...
    if(currentContext==='server') {
      if(tab==='Invite') return renderServerInvite(body);
      return renderServerMembers(body);
    }
    if(tab==='Invite') return renderChannelInvite(body);
    return renderChannelMembers(body,prefetched);
  }
  window.scmTab=scmTab;

  async function renderServerInvite(body) {
    const sid=currentServer?.id||activeServerId();
    body.innerHTML=`<div class="scm-muted" style="margin-bottom:10px">Create a real invite link. Anyone using it will join this server as a regular member.</div><div class="scm-grid"><input id="scmMaxUses" class="scm-input" type="number" min="0" value="0" placeholder="Max uses (0 = unlimited)"><input id="scmExpires" class="scm-input" type="number" min="0" value="0" placeholder="Expires in hours (0 = never)"></div><button class="scm-btn" onclick="window.scmCreateServerInvite()">🔗 Create Invite Link</button><div class="scm-section">Latest invites</div><div id="scmInviteList"><div class="scm-empty">Loading…</div></div>`;
    window.scmCreateServerInvite=async()=>{
      try{const d=await request('/API/server/invite.php','create',{},'POST',{server_id:sid,max_uses:Number(document.getElementById('scmMaxUses').value||0),expires_hours:Number(document.getElementById('scmExpires').value||0)}); renderCreatedInvite(body,d.invite.invite_url); await loadServerInvites();}catch(e){toast(e.message,'info')}
    };
    await loadServerInvites();
  }
  async function loadServerInvites(){
    const el=document.getElementById('scmInviteList'); if(!el)return;
    try{const d=await request('/API/server/invite.php','list',{server_id:currentServer?.id||activeServerId()}); if(!d.invites.length){el.innerHTML='<div class="scm-empty">No invites created yet.</div>';return;} el.innerHTML=d.invites.map(i=>`<div class="scm-row"><div class="scm-info"><div class="scm-name">Invite #${i.id}</div><div class="scm-meta">${i.use_count}${i.max_uses?' / '+i.max_uses:' uses'} · ${i.revoked_at?'Revoked':(i.expires_at?'Expires '+esc(i.expires_at):'Never expires')}</div></div>${!i.revoked_at?`<button class="scm-btn red" onclick="window.scmRevokeServerInvite(${i.id})">Revoke</button>`:''}</div>`).join('');
      window.scmRevokeServerInvite=async id=>{try{await request('/API/server/invite.php','revoke',{},'POST',{invite_id:id});loadServerInvites();}catch(e){toast(e.message,'info')}};
    }catch(e){el.innerHTML=`<div class="scm-empty">${esc(e.message)}</div>`}
  }
  async function renderServerMembers(body){
    const sid=currentServer?.id||activeServerId();
    body.innerHTML=`<div class="scm-grid"><input id="scmMemberSearch" class="scm-input" placeholder="Search users to add…"><button class="scm-btn" onclick="window.scmSearchCandidates()">Search</button></div><div id="scmCandidates"></div><div class="scm-section">Current members</div><div id="scmMembers"><div class="scm-empty">Loading…</div></div>`;
    document.getElementById('scmMemberSearch').addEventListener('keydown',e=>{if(e.key==='Enter')window.scmSearchCandidates()});
    window.scmSearchCandidates=async()=>{const q=document.getElementById('scmMemberSearch').value.trim();try{const d=await request('/API/server/members.php','candidates',{server_id:sid,q});document.getElementById('scmCandidates').innerHTML=d.users.length?d.users.map(u=>memberRow(u,true)).join(''):'<div class="scm-empty">No users found.</div>';}catch(e){toast(e.message,'info')}};
    window.scmAddMember=async id=>{try{await request('/API/server/members.php','add',{},'POST',{server_id:sid,user_id:id});toast('Member added','success');window.scmSearchCandidates();loadServerMembers();}catch(e){toast(e.message,'info')}};
    window.scmRemoveMember=async id=>{if(!confirm('Remove this member from the server?'))return;try{await request('/API/server/members.php','remove',{},'POST',{server_id:sid,user_id:id});toast('Member removed','success');loadServerMembers();}catch(e){toast(e.message,'info')}};
  ...
  async function handleInviteQuery(){
    const url=new URL(window.location.href); const serverToken=url.searchParams.get('invite'); const channelToken=url.searchParams.get('channel_invite'); if(!serverToken&&!channelToken)return;
    try{
      ...
      ['invite','channel_invite'].forEach(k=>url.searchParams.delete(k)); history.replaceState({},'',url.toString());
  ...
  function addHeaderActions(){
    ...
    if(ws&&!document.getElementById('scmServerBtns')){
      const box=document.createElement('div');box.id='scmServerBtns';box.className='scm-top-actions';box.innerHTML='<button class="scm-mini-btn" title="Invite to server">🔗</button><button class="scm-mini-btn" title="Manage members">👥</button>';
      box.children[0].onclick=e=>{e.stopPropagation();openServerManager('Invite')};box.children[1].onclick=e=>{e.stopPropagation();openServerManager('Members')};ws.appendChild(box);
    }
    ...
    if(header&&!document.getElementById('scmChannelInviteBtn')){
      ...
      const memberBtn=document.querySelector('.header-members'); if(memberBtn)header.insertBefore(b,memberBtn); else header.appendChild(b);
    }
  }
  ...
  function boot(){ensureStyles();addHeaderActions();patchPeerMatches();handleInviteQuery();
    const observer=new MutationObserver(()=>addHeaderActions()); observer.observe(document.body,{subtree:true,childList:true});
    setInterval(()=>{if(document.getElementById('matchesList')&&typeof window.refreshMatches==='function')window.refreshMatches();},60000);
  }
