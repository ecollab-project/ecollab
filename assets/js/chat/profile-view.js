(function(){
  ...
  function ensureStyles(){
    ...
    const s=document.createElement('style');s.id='ec-profile-view-styles';s.textContent=`
      ...
      .ec-profile-right{width:350px;background:#2b2d31;border-left:1px solid rgba(255,255,255,.06);padding:24px;overflow:auto}.ec-right-section{margin-top:18px}.ec-right-title{font-size:10px;text-transform:uppercase;letter-spacing:.09em;color:#949ba4;font-weight:800;margin-bottom:9px}.ec-right-connection{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px;border-radius:10px;background:#1e1f22;border:1px solid #45474d}.ec-right-connection strong{font-size:12px;color:#fff}.ec-right-connection span{font-size:10px;color:#949ba4}.ec-right-server-list{display:grid;gap:8px}.ec-right-server{padding:10px 12px;border-radius:9px;background:#36383e;border:1px solid #45474d;font-size:11px;font-weight:700;color:#fff}.ec-right-compat{padding:12px;border-radius:10px;background:linear-gradient(145deg,#24262b,#1e1f22);border:1px solid rgba(192,132,252,.28)}.ec-board-tabs{display:flex;gap:18px;border-bottom:1px solid #45474d;margin-bottom:18px}.ec-board-tab{padding:0 0 10px;color:#949ba4;font-size:12px;font-weight:700}.ec-board-tab.active{color:#fff;border-bottom:2px solid #fff}.ec-widget-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.ec-widget{min-height:92px;border-radius:10px;background:#36383e;border:1px solid #45474d;padding:12px;display:flex;align-items:flex-end;font-size:12px;font-weight:700;color:#fff;position:relative;overflow:hidden}.ec-widget::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(88,101,242,.25),rgba(236,72,153,.1));opacity:.7}.ec-widget span{position:relative;z-index:1}.ec-empty-board{font-size:12px;line-height:1.5;color:#949ba4;background:#36383e;border:1px solid #45474d;border-radius:10px;padding:16px}
    ...
    m.innerHTML=`<div class="ec-profile-card" role="dialog" aria-modal="true">
      ...
      <main class="ec-profile-main"><div class="ec-profile-banner" id="ecProfileBanner"><button class="ec-profile-close" id="ecProfileClose">×</button></div>
        <div class="ec-profile-content"><div class="ec-profile-avatar" id="ecProfileAvatar">?</div>
          ...
          <section class="ec-profile-section"><h4>Study Profile</h4><div class="ec-profile-stats"><div class="ec-stat"><b id="ecProfileStyle">—</b><span>Study Style</span></div><div class="ec-stat"><b id="ecProfileGoal">—</b><span>Goal</span></div><div class="ec-stat" id="ecMatchStat"><b id="ecProfileCompat">—</b><span>Match</span></div></div></section>
      ...
      <aside class="ec-profile-right"><div class="ec-board-tabs"><div class="ec-board-tab active">Board</div><div class="ec-board-tab">Activity</div><div class="ec-board-tab">Wishlist</div></div><div id="ecProfileBoard"></div><div class="ec-right-section"><div class="ec-right-title">Study Profile</div><div class="ec-profile-stats"><div class="ec-stat"><b id="ecRightStyle">—</b><span>Study Style</span></div><div class="ec-stat"><b id="ecRightGoal">—</b><span>Goal</span></div><div class="ec-stat"><b id="ecRightMatch">—</b><span>Match</span></div></div></div><div class="ec-right-section"><div class="ec-right-title">Study Activity</div><div class="ec-profile-stats"><div class="ec-stat"><b id="ecRightStreak">—</b><span>Streak</span></div><div class="ec-stat"><b id="ecRightHours">—</b><span>Study Hours</span></div><div class="ec-stat"><b id="ecRightMutualCount">—</b><span>Mutual Servers</span></div></div></div><div class="ec-right-section" id="ecRightCompatSection"><div class="ec-right-title">Compatibility</div><div class="ec-right-compat"><div class="ec-compat-head"><span>Match with you</span><span class="ec-compat-score" id="ecRightCompatTotal">—</span></div><div class="ec-compat-bars" id="ecRightCompatBars"></div></div></div><div class="ec-right-section"><div class="ec-right-title">Connection</div><div class="ec-right-connection" id="ecRightConnection"></div></div><div class="ec-right-section"><div class="ec-right-title">Mutual Servers</div><div class="ec-right-server-list" id="ecRightServers"></div></div></aside>
    </div>`;
  ...
  async function open(target){
    ...
    try{
      ...
      document.getElementById('ecProfileStyle').textContent=p.study_style||'—';document.getElementById('ecProfileGoal').textContent=p.goals||'—';document.getElementById('ecProfileCompat').textContent=p.compatibility_score!=null?`${p.compatibility_score}%`:'—';const cb=p.compatibility_breakdown;document.getElementById('ecProfileStreak').textContent=p.streak_days?`${p.streak_days}d`:'—';document.getElementById('ecProfileHours').textContent=p.study_hours?`${p.study_hours}h`:'—';document.getElementById('ecProfileServers').textContent=Array.isArray(p.mutual_servers)?p.mutual_servers.length:'0';
      document.querySelectorAll('.ec-profile-nav').forEach((el,i)=>{el.style.cursor='pointer';el.onclick=()=>{document.querySelectorAll('.ec-profile-nav').forEach(n=>n.classList.remove('active'));el.classList.add('active');if(i===0)return;if(i===1)document.querySelector('.ec-profile-section:last-of-type')?.scrollIntoView({behavior:'smooth',block:'center'});if(i===2&&Number(p.id)!==Number(window.ECOLLAB?.userId))openDM(p.id,p.full_name||p.username,p.avatar_color_gradient||'');if(i===3)document.getElementById('ecProfileBoard')?.scrollIntoView({behavior:'smooth'});if(i===4&&typeof window.openPeerMatchingModal==='function'){close();window.openPeerMatchingModal();}}});const ms=document.getElementById('ecMatchStat');if(ms)ms.onclick=()=>{if(typeof window.openPeerMatchingModal==='function'){close();window.openPeerMatchingModal();}};const board=document.getElementById('ecProfileBoard');const servers=Array.isArray(p.mutual_servers)?p.mutual_servers:[];board.innerHTML=servers.length?`<div class="ec-widget-grid">${servers.slice(0,6).map(s=>`<div class="ec-widget"><span>⭐ ${esc(s)}</span></div>`).join('')}</div>`:`<div class="ec-empty-board">${p.id===Number(window.ECOLLAB?.userId)?'Your profile is ready. Use Dashboard to manage your study workspace.':'No mutual servers yet.'}</div>`;document.getElementById('ecRightStyle').textContent=p.study_style||'—';document.getElementById('ecRightGoal').textContent=p.goals||'—';document.getElementById('ecRightMatch').textContent=p.compatibility_score!=null?p.compatibility_score+'%':'—';document.getElementById('ecRightStreak').textContent=p.streak_days?p.streak_days+'d':'—';document.getElementById('ecRightHours').textContent=p.study_hours?p.study_hours+'h':'—';document.getElementById('ecRightMutualCount').textContent=servers.length;const rightCompat=document.getElementById('ecRightCompatSection'),rightTotal=document.getElementById('ecRightCompatTotal'),rightBars=document.getElementById('ecRightCompatBars');if(Number(p.id)!==Number(window.ECOLLAB?.userId)&&p.compatibility_score!=null&&cb){rightCompat.style.display='block';rightTotal.textContent=p.compatibility_score+'%';rightBars.innerHTML=[['Subjects',cb.subjects],['Study style',cb.style],['Interests',cb.interests],['Hobbies',cb.hobbies]].map(([label,val])=>'<div class="ec-compat-row"><span>'+label+'</span><div class="ec-compat-track"><div class="ec-compat-fill" style="width:'+Math.max(0,Math.min(100,Number(val)||0))+'%"></div></div><b>'+Math.round(Number(val)||0)+'%</b></div>').join('');}else{rightCompat.style.display='none';}const rc=document.getElementById('ecRightConnection');const status=p.connection_status||'none';rc.innerHTML='<div><strong>'+(Number(p.id)===Number(window.ECOLLAB?.userId)?'Your account':status==='accepted'?'Connected':status==='pending'?'Request pending':'Not connected')+'</strong><br><span>'+(status==='accepted'?'You can message each other directly.':status==='pending'?'A connection request is waiting for a response.':'Connect to collaborate more easily.')+'</span></div>';if(Number(p.id)!==Number(window.ECOLLAB?.userId)&&status==='accepted'){const b=document.createElement('button');b.className='ec-pbtn primary';b.textContent='Message';b.onclick=()=>openDM(p.id,p.full_name||p.username,p.avatar_color_gradient||'');rc.appendChild(b);}const rs=document.getElementById('ecRightServers');rs.innerHTML=servers.length?servers.map(s=>'<div class="ec-right-server">⭐ '+esc(s)+'</div>').join(''):'<div class="ec-empty-board">No mutual servers yet.</div>';
