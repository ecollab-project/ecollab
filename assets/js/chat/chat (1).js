...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const firstChannel = document.querySelector('.channel-item[data-channel-id]');
  if (firstChannel) {
    switchChannel(firstChannel, parseInt(firstChannel.dataset.channelId));
  }
...
function renderChannelList(channels) {
  ...
  channels.forEach(ch => {
    ...
    if (ch.type === 'voice') {
      ...
      el.innerHTML = `
        ${isAnnouncement
          ? `<svg width="13" height="13" fill="currentColor" viewBox="0 0 24 24" style="color:var(--accent-yellow);flex-shrink:0;"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6V11c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>`
...
async function switchChannel(el, channelId) {
  ...
  try {
    ...
    if (chanData.channel) {
      document.getElementById('channelTitle').textContent = chanData.channel.name;
      document.getElementById('channelDesc').textContent = chanData.channel.description || '';
      document.getElementById('chatInputField').placeholder = `Message #${chanData.channel.name}`;
      document.getElementById('mobChannelName').textContent = chanData.channel.name;
      // Hide input for non-text channel types
      const inputWrapper = document.getElementById('chatInputWrapper');
      const isTextChannel = ['text', 'study_room'].includes(chanData.channel.type);
...
function showMsgMenu(event, btn, author, msgId, isMe) {
  ...
  if (isMe) {
    ...
    items.push({ sep: true });
    items.push({ icon: '🚩', label: 'Report Message', red: true, action: () => showToast('🚩 Reported', 'info') });
  }
...
async function saveEditMsg(el, msgId) {
  ...
  try {
    await apiFetch(`${API_BASE}/edit-message.php`, {
      method: 'POST',
...
function renderMembersPanel(members) {
  ...
  const html = members.slice(0, 20).map(m => {
    ...
    return `
      <div class="member-item" onclick="openMiniProfile(event, '${escHtml(m.full_name || m.username)}', '${escHtml(m.role || 'Student')}', '', '${init}')">
...
// openExtrasAction is defined in chat-features.js — bind it after all scripts load
window.openExtrasAction = function (...args) {
  if (typeof openExtrasAction === 'function') return openExtrasAction(...args);
};
