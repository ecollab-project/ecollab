...
class OtpService {
    ...
    /**
     * Send the plaintext OTP to the user's email through Brevo's
    ...
    public function deliver(string $toEmail, string $toName, string $otp, string $action = 'reset_password'): array {
        ...
        if (BREVO_API_KEY === '' || BREVO_SENDER_EMAIL === '') {
            error_log('[OtpService] Brevo API mail settings are not configured.');
        ...
        $payload = json_encode([
            ...
            'subject'     => $subject,
            'textContent' => strip_tags(str_replace(['</p>', '<br>', '<br/>', '<br />'], ["\n", "\n", "\n", "\n"], $body)),
            'htmlContent' => $body,
        ], JSON_UNESCAPED_SLASHES);

        if ($payload === false) {
            error_log('[OtpService] Failed to encode Brevo API payload.');
            return ['success' => false, 'error' => 'Unable to prepare email.'];
        }

        $ch = curl_init(rtrim(BREVO_API_URL, '/') . '/v3/smtp/email');
        if ($ch === false) {
