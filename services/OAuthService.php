...
class OAuthService
{
    ...
    public function handleCallback(string $provider, string $code, string $state): array
    {
        ...
        try {
            ...
            // Students created through SSO have only a minimal profile initially.
            // Keep them authenticated, but send them through signup steps 2-5
            // until their academic profile has been completed.
            if ($user['role'] === 'student' && !$this->hasCompletedOnboarding((int)$user['id'])) {
                $_SESSION['oauth_onboarding'] = true;
                $_SESSION['oauth_onboarding_provider'] = $provider;

                return [
                    'success' => true,
                    'redirect' => BASE_URL . '/modules/auth/signup.php?oauth_onboarding=1'
                ];
            }

            unset($_SESSION['oauth_onboarding'], $_SESSION['oauth_onboarding_provider']);
    ...
    private function hasCompletedOnboarding(int $userId): bool
    {
        $stmt = $this->db->prepare("
            SELECT academic_program_id, year_level
            FROM user_profiles
            WHERE user_id = :uid
            LIMIT 1
        ");
        ...
        return $profile
            && !empty($profile['academic_program_id'])
            && (int)$profile['year_level'] >= 1;
    }
