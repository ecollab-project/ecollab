...
final class CoworkspaceService
{
    public static function get(PDO $db, int $workspaceId, int $userId, bool $requireAccess = true): array
    {
        ...
        if ($requireAccess) {
            ...
            // Public Coworkspaces are joinable by server members. Treat a
            // server member who has not yet been persisted in the member table
            // as a normal member for access/permission checks. Private spaces
            // still require an explicit workspace membership row.
