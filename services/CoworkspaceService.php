...
final class CoworkspaceService
{
    ...
    public static function resolveWhiteboardAccess(PDO $db, int $whiteboardId, int $channelId, int $userId): ?array
    {
        ...
        if ((int)$board['channel_id'] !== $channelId) return null;
        if (!empty($board['archived'])) return null;
        ...
        if (!$isHost && $memberRole === null && $workspaceVisibility !== 'public') {
            return null;
        }

        $stmt = $db->prepare(
            'SELECT permission
             FROM collab_resource_permissions
             WHERE resource_type = "whiteboard"
               AND resource_id = :rid
               AND user_id = :uid
             LIMIT 1'
        );
        $stmt->execute([':rid' => $whiteboardId, ':uid' => $userId]);
        ...
        $effective = $isOwner
            ...
            : ($explicit !== null
                ? $explicit
                : ($board['visibility'] === 'public' ? $publicPermission : null));
